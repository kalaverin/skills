[ref: #dcc-project-template]

# Project-level dependency card template
...
2. Every entity in `entities/` has a fresh `logic/<entity>/dependencies.md`
   card whose frontmatter commit matches the entity's repository HEAD.
...
`repo` is `serena` because the source of truth for this project-wide memory is
`.serena`, per `[ref: #serena-metadata-contract]`.
...
**Scope:** project-level overview of services, their taxonomy, dependency edge
 ...
 cross-domain summary. Method-level details live in `logic/<entity>/dependencies.md`.
...
- Point to `logic/<entity>/dependencies.md` for method/endpoint/workflow details.
- Explain that the overview is derived from `entities/` cards and
...
| Alias / deprecated name | Canonical `entities/` card | Notes |
|---|---|---|
| `old-billing-api` | *(none — external)* | Deprecated in-project service. Active billing is the external `billing` monolith. |
| `case-management` | `entities/dispute_api` | Proto package `classic.case_management`. |
...
Columns: `entities/ card`, `Directory`, `Type`, `Runtime role`, `Dependency card`.

Example row:

| `entities/` card | Directory | Type | Runtime role | Dependency card |
|---|---|---|---|---|
| `wallet_wf` | `wallet-wf/` | Temporal workflow worker | Deposit / topup / withdrawal / clearing | `logic/wallet_wf/dependencies.md` |

Every entity in `entities/` should appear here unless explicitly skipped and
...
- Stub/non-functional entities.
- Method-level details live in `logic/<entity>/dependencies.md`.
...
- No "Dependency graph by service" section — that belongs in each
  `logic/<entity>/dependencies.md`.
- Every entity in `entities/` is either in the taxonomy or explicitly skipped
  with a reason.
