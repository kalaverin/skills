...
index:
  - anchor: frameworks-django
    ...
    problem: "Django suite written with plain pytest idioms hits `DatabaseOperation` blockage because plugin keeps ORM access off unless explicitly granted, and every test's writes vanish inside rolled-back atomic block by default; query count budget, commit callback capture, temporary settings override, alternate urlconf, reuse database, per-test mail capture."
    use_when: "Suite adopts pytest style over `TestCase` subclasses; test touches ORM and needs declared database access via mark or fixture; transaction behavior, query budget, mail, settings, URLconf, or on-commit side effects under assertion."
    avoid_when: "Query-count block stretched into coverage target instead of deliberate invariant; mail asserted through global `mail.outbox` rather than `mailoutbox` fixture; `settings.ROOT_URLCONF` mutated directly instead of `@pytest.mark.urls`."
    expected: "Every ORM-touching test declares database access explicitly and runs isolated in rolled-back transactions, with mail, settings, URLconf, callbacks, and query budgets asserted through auto-reverting fixtures and markers."
  - anchor: frameworks-django-variety-booster
    ...
    problem: "Authorization and routing checks across staff roles and HTTP methods duplicate test bodies when each combination gets its own test, and hand-written identifiers collide inside session; staff versus member access, crud sweep, indirect fixture param, colliding slug, per-case status."
    use_when: "View permission or CRUD behavior varies by role and method; expected status differs per case; generated identifiers must stay unique across session-wide parametrized runs."
    avoid_when: "One test body cloned per role/method/status combination; identifiers hand-rolled so they can collide within session instead of `fake.unique.slug()` or equivalent unique provider."
    expected: "Single parametrized body covers staff and non-staff across all relevant HTTP methods with correct status per case, while factory-built fixtures keep identifiers unique throughout session."
  - anchor: frameworks-fastapi
    ...
    problem: "Endpoint suite needs real HTTP behavior with startup and shutdown handlers honored, yet naive transport skips lifespan events and shared async fixtures explode with cross-loop attachment errors; missing lifespan kickoff, different loop task, unrestored swap, schema drift, unvalidated payload."
    use_when: "Endpoints must be exercised over HTTP rather than direct function calls; startup or shutdown handlers initialize state; dependencies need swapping without patching database layer; responses require schema-level validation including rejection paths."
    avoid_when: "`ASGITransport` assumed to fire lifespan events unwrapped; sync `TestClient` placed inside async test or async fixtures shared across loops; `app.dependency_overrides` left mutated after test; database layer patched instead of dependency swap; only well-formed responses validated."
    expected: "Sync and async endpoints run over HTTP with lifecycle handlers executed and every test on its own loop, overrides revert cleanly, and payloads validate against schemas on acceptance and rejection paths."
  - anchor: frameworks-fastapi-variety-booster
    ...
    problem: "Role-by-endpoint authorization checks multiply test bodies when each pairing gets hand-written test, and leaked override state bleeds between cases; role matrix explosion, per-route schema check, override hygiene, shared body, teardown restore."
    use_when: "Authorization outcome depends on role across several routes; response schema must hold per endpoint; overrides need guaranteed restoration between parametrized cases."
    avoid_when: "Separate user records created per role instead of override matrices; `app.dependency_overrides` left mutated beyond test body; schema validation skipped on parametrized paths."
    expected: "One parametrized body asserts every role's authorization outcome and each endpoint's schema, with overrides reverted so no fake user or role leaks into later cases."
  - anchor: frameworks-flask
    ...
    problem: "Flask suite built on globally constructed app with hardcoded secrets leaks state between tests and cannot vary configuration, while session and CLI paths stay unreachable without proper harness; session pre-seed, upload tuple form, cli runner invocation, redirect history assertion, request context lifetime, testing flag isolation."
    use_when: "App can be constructed through factory with injected config; routes, sessions, uploads, redirects, or CLI commands must run against isolated instance; secrets and database URIs must be generated rather than hardcoded."
    avoid_when: "Singleton module-level app under test rather than factory-built instance; secrets or database URIs hardcoded instead of generated with `TESTING=True`; session accessed outside kept-alive request context (`with client:` plus `session_transaction`)."
    expected: "Isolated factory-built app with `TESTING=True` and generated credentials exercises HTTP routes, sessions, uploads, redirects, and command-line entry points through client and runner without leaking global state."
  - anchor: frameworks-flask-variety-booster
    ...
    problem: "Configuration variants and role-based access rules force cloned route tests when each flag, backend, or role gets its own body, and redirect outcomes split away from success checks; config-per-test duplication, anonymous member admin sweep, seeded role session, feature flag matrix."
    use_when: "Behavior varies across configurations (feature flags, database backends) or across anonymous/member/admin roles; one route body could serve every variant through parametrized fixtures; success and authorization-redirect statuses deserve unified assertion."
    avoid_when: "Distinct route bodies maintained per configuration or per role; redirect outcome asserted apart from success outcome although one parametrized status covers both; session-seeded clients rebuilt from scratch instead of derived from base `client`."
    expected: "Single route body exercises every configuration and role, authorization redirects included, with assertions shared and variants injected through fixtures rather than duplicated tests."
libraries:
  - asgi-lifespan
  - click
  - django
