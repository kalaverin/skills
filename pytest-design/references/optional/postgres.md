---
subject: "Substitute PostgreSQL with SQLite at runtime for fast isolated SQLAlchemy 2.0 async tests; import-time patches for JSONB, Uuid.bind_processor, and psycopg.errors.UniqueViolation, NullPool + begin_nested savepoint, postgresql_where index stripping, Faker factories, flush+refresh persist helper, sample fixtures, repository/service/gRPC servicer tests, real versus mock IntegrityError, zero-warnings policy, anti-patterns checklist, troubleshooting map, copy-paste conftest template."
index:
  - anchor: postgres-surrogate-approach
    ...
    problem: "Async repositories, services, and gRPC servicers hit production PostgreSQL through SQLAlchemy 2.0, yet full server plus Docker makes every test slow and brittle; mocked repositories test nothing real while container startup multiplies cost; persistence layer, real sql execution, runtime overhead, mock inadequacy, isolation boundary, fast feedback."
    use_when: "The production stack is SQLAlchemy 2.0 async on PostgreSQL with psycopg; tests target repositories, services, or gRPC servicers; real SQL execution matters more than PostgreSQL-specific behavior; Docker or a running server is too slow for the loop."
    avoid_when: "The test exercises PostgreSQL-only behavior such as JSONB operators, full-text search, ranges, custom types, stored procedures, or query plans; mocking the repository is acceptable; a real PostgreSQL instance is available and fast enough."
  ...
  - anchor: postgres-jsonb-to-json
    ...
    problem: "Models declare `sqlalchemy.dialects.postgresql.JSONB` columns at import time, so `metadata.create_all` on `SQLite` raises unsupported type before any test runs; model imports that run before dialect replacement still explode; column type mismatch, import ordering, dialect type, metadata build, unsupported construct, schema create."
    use_when: "Models use `sqlalchemy.dialects.postgresql.JSONB`; the surrogate runs on `SQLite`; model imports can be ordered after the dialect patch."
    avoid_when: "The patch is placed after model imports (it is silently ignored); the test must assert real JSONB operator behavior; JSONB is not used in models."
  ...
  - anchor: postgres-uuid-bind-processor
    ...
    avoid_when: "Primary and foreign keys already share one representation; the original processor is not restored after the session; the mismatch does not occur on the target database."
  ...
  - anchor: postgres-unique-violation-alias
    ...
    avoid_when: "The alias is applied unconditionally when `psycopg` is present and its real exception is required; the original exception class is not restored afterward; the test does not exercise duplicate handling."
  ...
  - anchor: postgres-restore-patches
    ...
    avoid_when: "Restoration is skipped and globals leak into later collection; `psycopg.errors.UniqueViolation` is restored when its saved original is `None`; patches are applied but never saved."
  ...
  - anchor: postgres-db-session
    ...
    problem: "One fixture hands repositories real `AsyncSession` against `SQLite` while bypassing `db.setup()` that would spawn `PostgreSQL` engine, and leaked patched globals or metadata corrupt later tests; session wiring, dsn override, ssl override, setup bypass, transaction manager, metadata restore, global rollback, function scope, leak-free teardown."
    ...
    avoid_when: "`db.setup()` is called inside the fixture or any test (it builds a `PostgreSQL` engine and overwrites the `SQLite` session); any patched global or metadata is leaked past `finally`; the fixture is shared across tests without rollback."
  ...
  - anchor: postgres-nested-transaction
    ...
    avoid_when: "The file or schema is recreated per test (slow); the next test is expected to clean up; `commit` is allowed to escape the savepoint."
  ...
  - anchor: postgres-strip-partial-indexes
    ...
    avoid_when: "Stripped indexes are left discarded (later tests see incomplete metadata); partial indexes are created on `SQLite`; metadata corruption is acceptable."
  ...
  - anchor: postgres-seeded-fake
    ...
    problem: "Factories need reproducible randomized data across locales, but unseeded generator makes flaky snapshots and hardcoded seeds remove entropy when environment variable is absent; seeded instance, locale list, env override, deterministic run, snapshot stability, entropy control."
    ...
    avoid_when: "Stdlib `random` is used anywhere except the one-time seed initialization; a fixed seed is hardcoded that kills entropy when no override is set; Faker is not used for test data."
  ...
  - anchor: postgres-model-factories
    ...
    problem: "Non-explicit fields come from a generator and unique-constrained columns cannot collide, yet hardcoded literals or direct `uuid.uuid4()` calls create collisions and non-reproducible runs; detached instance, default generation, unique field, collision free, factory override, seeded data, no literal, domain object."
    ...
    avoid_when: "Hardcoded literals like `'test_user'` are used; `uuid.uuid4()` or stdlib `random` are called; the factory persists inside itself instead of returning detached instances."
  ...
  - anchor: postgres-add-item
    ...
    problem: "Helper that persists inside shared session must make row visible and hydrate primary key, but calling `commit` ends savepoint early and leaks row past rollback into sibling tests; flush only, refresh identity, no commit, nested rollback, visibility inside transaction, leak prevention, persistence helper."
    ...
    avoid_when: "`commit` is called inside helper fixtures (breaks nested-transaction isolation); `refresh` is skipped when the test reads generated primary keys; the helper creates its own session."
  ...
  - anchor: postgres-sample-fixtures
    ...
    avoid_when: "The fixed-name exception extends beyond true domain concepts (channel types, roles, statuses); every other field does not use Faker defaults; the dataset is reused across tests without rebuild."
  ...
  - anchor: postgres-repository-tests
    ...
    avoid_when: "The repository is mocked (tests the mock, not the SQL); assertions rely on driver internals like `rowcount`; the test does not need real SQL execution."
  ...
  - anchor: postgres-service-tests
    ...
    problem: "Service resolves session through `StorageFactory` and `TransactionManager` rather than explicit argument, so unpatched factory would hand it production session and bypass surrogate entirely; factory wiring, hidden session, transaction manager, service layer, implicit dependency, routing, integration seam, context bound, factory swap."
    ...
    avoid_when: "`StorageFactory.tr_manager` is left unpatched (service reaches production); a session is passed explicitly when the service resolves it implicitly; the factory wiring is not verified."
  ...
  - anchor: postgres-grpc-servicer
    ...
    problem: "gRPC servicer chains service into `StorageFactory` into surrogate, so full request path runs without server while only external context stands as `AsyncMock` stub; end-to-end servicer, request response, context stub, no running server, rpc handler, transport-free, unary call, populated message, handler entry."
    ...
    avoid_when: "A real gRPC server is started; repositories or services beneath the servicer are mocked (only the context is mocked); the servicer is tested in isolation from the data layer."
  ...
  - anchor: postgres-unique-constraint
    ...
    problem: "Plain `UNIQUE` column is enforced by `SQLite` and should raise real `IntegrityError` on duplicate insert, yet broad `except` or wrong type hides regression and lets duplicate through; real constraint, duplicate insert, integrity error, explicit exception, match pattern, no broad catch, uniqueness enforcement, regression signal."
    ...
    avoid_when: "The constraint is partial or uses a custom type `SQLite` cannot enforce; `except` is too broad or asserts a wrong exception type; `flush` is mocked for a constraint `SQLite` could enforce for real."
  ...
  - anchor: postgres-unique-constraint
    ...
    problem: "Partial unique index stripped for `SQLite` cannot fire real violation, so repository mapping from integrity error to domain `AlreadyExistsError` stays unreachable unless `session.flush` is patched to raise; partial predicate, unenforceable constraint, simulated violation, domain error mapping, translation path, fallback only, last resort, mapped exception, surrogate gap."
    ...
    avoid_when: "Mock-flush is used for non-partial constraints (insert real duplicates instead); it is treated as the default rather than a last-resort fallback; the patched `flush` raises a wrong exception type."
  ...
  - anchor: postgres-assert-domain
    ...
    avoid_when: "`self.assertEqual`/`assertTrue`/`assertRaises` are used; assertions target `rowcount` or other `SQLite`-internal state; the test is tied to surrogate internals."
  ...
  - anchor: postgres-factory-overrides
    ...
    problem: "Single test needs one specific field set while remaining fields should vary, but hardcoding every column removes randomness and masks collision or formatting bugs; explicit override, partial literal, generated rest, targeted value, entropy preserved, domain enum, fixed status, boundary input, named argument, selective control."
    ...
    avoid_when: "Every field is hardcoded (removes entropy, hides collisions); the factory is bypassed to hand-build instances inline; the overridden field is not a true boundary input."
  ...
  - anchor: postgres-time-control
    ...
    problem: "Expiry and timestamp logic depends on frozen clock, but `time.sleep` or `asyncio.sleep` slows suite and real clock makes past-and-future assertions nondeterministic; clock travel, tick false, expiry check, deterministic instant, no sleep, timestamp logic, created at, past future."
    ...
    avoid_when: "`time.sleep` or `asyncio.sleep` is used; the real wall clock is relied upon for time-sensitive assertions; time is not a factor in the behavior under test."
  ...
  - anchor: postgres-zero-warnings
    ...
    problem: "Suite output should carry zero warnings, yet `ResourceWarning` from unclosed engine or `PytestUnknownMarkWarning` from unregistered marker slips in unless treated as error and fixed at root; warnings as errors, resource leak, unknown mark, root cause, narrow filter, upstream ticket, no blanket ignore, clean output, deprecation fix."
    ...
    avoid_when: "Blanket `warnings.filterwarnings('ignore')` or `pytest.mark.filterwarnings('ignore')` is used; warnings are suppressed without exact class, module regex, and a ticket link; warnings are treated as optional noise."
  ...
  - anchor: postgres-anti-patterns
    ...
    avoid_when: "Any listed anti-pattern is treated as acceptable; `gc.collect()` is relied on as a substitute for explicit `close()`/`dispose()`; the checklist is skipped."
  ...
  - anchor: postgres-troubleshooting
    ...
    problem: "Failure symptom like `database is locked` or UUID mismatch needs fast map to cause and fix, because guessing wastes cycles and same errors recur across projects adopting surrogate; symptom table, root cause, fix lookup, locked database, type mismatch, dsn leak, diagnostic map, recurring error, quick triage."
    use_when: "A test failure matches a known surrogate symptom; the goal is fast triage to root cause and the matching fix."
    avoid_when: "Causes are guessed at; symptoms are papered over with broad ignores instead of applying the mapped fix; the table is not updated with new project-specific failures."
  ...
  - anchor: postgres-adapt-pattern
    ...
    problem: "Porting surrogate to another codebase requires auditing models for dialect-only constructs before writing fixtures, because missed `ARRAY` or `INET` column or extra partial index surfaces only at `create_all`; port steps, model audit, dialect construct, index inventory, fixture rebuild, project transfer, adaptation order, pre-flight scan, column inventory, extension check."
    ...
    avoid_when: "Fixtures are written before auditing models (missed dialect constructs explode at `create_all`); the per-construct patch decision is skipped; dialect-only constructs are ignored."
  ...
  - anchor: postgres-copy-paste-template
    ...
    avoid_when: "Model modules are imported before the import-time patches; any save/restore pair is omitted from the skeleton; the template is modified to change the patch-import order."
