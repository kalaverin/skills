...
index:
  - anchor: exceptions-raises-match
    ...
    problem: "Asserting only exception type passes when right type carries wrong message, so message fragment must pin exact failure; type and message, specific fragment, escape literal, broad pattern trap, re search, unrelated message fails."
    use_when: "Exception type alone passes wrong message, message fragment pins exact failure, and literal regex metacharacters need escaping; type and message, specific fragment, escape literal, broad pattern trap, re search, unrelated message fails, raises match."
    avoid_when: "Do not use broad patterns such as `match='.*'` or `match='error'`, which mask unintended exceptions."
    expected: "The test passes only when the exact exception type is raised with a message containing the specific fragment, and unrelated messages fail."
  - anchor: exceptions-exc-info
    ...
    problem: "Some exceptions carry structured state (custom attributes, args) that message fragment cannot assert; type identity, custom attribute, structured args, traceback, typename trap, beyond message."
    use_when: "Exception carries structured state beyond message, custom attributes args or traceback matter, and typename check would mislead; type identity, custom attribute, structured args, traceback, typename trap, beyond message, exc info inspect."
    avoid_when: "Do not assert on `exc_info.typename` for type checks — it is a mostly-informational string; use `exc_info.type is ExpectedException`."
  ...
  - anchor: exceptions-parametrize
    ...
    problem: "Hand-writing one test per invalid input duplicates same raises boilerplate and discourages adding failure modes; shared exception pattern, control labels, generate values, one body many inputs, add mode by label, no duplicate raises."
    use_when: "Many invalid inputs share exception pattern, hand written tests duplicate raises boilerplate, and labels add failure modes cleanly; shared exception pattern, control labels, generate values, one body many inputs, add mode by label, no duplicate raises."
    avoid_when: "Do not duplicate near-identical `pytest.raises` tests for each invalid input when a parametrized label set would cover them with one body."
  ...
  - anchor: exceptions-warnings
    what: "`pytest.deprecated_call()` asserts a block emits a `DeprecationWarning`/`PendingDeprecationWarning`, and `pytest.warns(WarningClass, match=...)` asserts a specific warning class and message fragment."
    problem: "Warning emission must assert without coupling tests to unrelated warning classes or to exception-raising tools; deprecation warning, specific class, message fragment, not exception tool, warning only, class precision."
    use_when: "Warning emission must assert precisely, unrelated warning classes or exception tools would couple test wrongly to implementation; deprecation warning, specific class, message fragment, not exception tool, warning only, class precision, warns match."
    avoid_when: "Never use these warning tools to assert expected exceptions; they are for warnings only."
  ...
  - anchor: exceptions-xfail
    ...
    problem: "Using xfail for intended behavior hides contract, while using raises for unfixed bug breaks suite on known-bad code; known bug document, enforce correct, issue link, strict xfail, contract not hidden, green suite."
    use_when: "Known bug needs documentation without breaking build, intended correct behavior needs enforcement, and strict xfail resurfaces fixes; known bug document, enforce correct, issue link, strict xfail, contract not hidden, green suite, raises versus xfail."
    avoid_when: "Do not use `xfail` as a substitute for `pytest.raises` when the exception is the intended contract."
    expected: "Known bugs are documented as xfail with an issue link while correct behavior is enforced by pytest.raises, so the suite stays green without hiding contracts."
  - anchor: exceptions-anti-patterns
    ...
    problem: "Teams repeatedly ship confidence-reducing exception assertions because no shared checklist of known-bad patterns exists; broad match, typename assert, broad exception, legacy callable form, bare except, review checklist."
    use_when: "Exception assertions repeatedly ship known bad patterns, review checklist is missing, and broad matches hide unrelated failures; broad match, typename assert, broad exception, legacy callable form, bare except, review checklist, exception review."
    avoid_when: "Avoid `match='.*'`, asserting `exc_info.typename`, catching broad `Exception` in `pytest.raises`, the legacy callable form `pytest.raises(Exc, func, *args, **kwargs)`, and bare `except:`/`except Exception:` in production code."
    expected: "Every exception assertion is specific (type, message fragment, or attribute) and would fail on an unintended exception, with the table serving as the review checklist."
  - anchor: exceptions-exception-group
    what: "Manually inspecting `exc_info.value.exceptions` to assert on the inner exceptions of an `ExceptionGroup` (Python 3.11+), with `pytest.RaisesGroup` available on pytest 8.0+."
    problem: "ExceptionGroup bundles multiple exceptions, so single type and message assertion cannot verify individual inner errors; inner exceptions, group inspect, nested errors, version gate, complete group, beyond single type."
    use_when: "ExceptionGroup bundles inner errors, single type and message assertion cannot verify individuals, and version gate decides helper; inner exceptions, group inspect, nested errors, version gate, complete group, beyond single type, raises group."
    avoid_when: "Do not assert on an `ExceptionGroup` as if it were a single exception (type+message only) when the inner exceptions are the real invariant."
