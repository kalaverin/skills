---
subject: "Generate realistic, deterministic data with Faker: built-in `faker` fixture, custom seeded multi-locale `fake` fixture (env-driven seed, sole permitted stdlib `random`), `faker.unique.<method>` for small suites vs deterministic suffixes for large ones, seed/locale overrides via `faker_seed`/`faker_locale`, factory fixtures, PII sanitization with `email(domain=...)`, mandatory Faker API audit test."
index:
  - anchor: faker-api-audit
    ...
    problem: "Faker API changes between releases without notice, so relying on memory risks calling renamed, removed, or differently-signed provider methods and suite breaks on upgrade; verify version, official docs, verified provider, todo fallback, api drift, semantic provider."
    use_when: "Selecting Faker providers for a test or factory; installed Faker version unknown or recently upgraded; a provider call comes from memory rather than verified docs."
    avoid_when: "Memory of the Faker API trusted across releases; generic `faker.word()`/`faker.text()` used unless the test genuinely needs unstructured text."
  ...
  - anchor: faker-unique-fields
    ...
    problem: "Unique-constrained columns collide when values are drawn randomly across many entities, and unique provider can raise on large draws, failing suite intermittently; collision risk, retry exhaustion, enumerated suffix, clear registry, small versus large suite."
    use_when: "Column carries a unique constraint and values are drawn randomly; collisions between generated values fail the test; batch size could exhaust `faker.unique` retries."
    avoid_when: "Large suites relying on `faker.unique` — retries can raise `UniquenessException`; unique registry left populated when reusing an instance within a test."
  ...
  - anchor: faker-locale-seeding
    ...
    problem: "Tests need reproducible isolated generated data with controlled locale instead of leaking uniqueness state or seed between tests; pinned seed, function scope, registry cleared, env seed, locale override, multilocale generation, per-test instance."
    use_when: "Generated data must be reproducible within a test run; locale or seed needs explicit control; uniqueness registry must not leak between tests."
    avoid_when: "Faker instance shared across tests at a scope wider than function (uniqueness state leaks); stdlib `random` used anywhere except the one permitted call in the custom `fake` fixture per `SKILL.md` #1.1.3."
  ...
  - anchor: faker-factory-fixtures
    ...
    problem: "Hand-assembling entities in every test duplicates field lists across suite and hides which fields matter to scenario under test; domain factory, override kwargs, specialize wrapper, shared base, verified methods, entity builder, field pinning."
    use_when: "Domain entity construction repeats the same field lists across tests; scenario-relevant fields stay hidden among defaults; overrides should pin only what matters."
    avoid_when: "Factories built from unverified Faker methods; field lists duplicated across specialized fixtures instead of composed from a base factory."
  ...
  - anchor: faker-pii-sanitization
    ...
    problem: "Real-looking generated emails and URLs can accidentally resolve or deliver, leaking data or hitting real inboxes; unroutable host, no real recipient, accidental delivery, inbox hit, dns resolution, address rewrite, reserved tld, plus alias."
    use_when: "Generated emails or URLs look real; accidental resolution or delivery would leak data or hit real inboxes; domains must be non-routable by construction."
    avoid_when: "Real-looking routable domains emitted (e.g. raw `company_email()`) in test data without sanitizing the domain."
  ...
  - anchor: faker-fake-tld-email-provider
    ...
    problem: "Entities with email fields (accounts, users, invitations, subscriptions) need realistic addresses that cannot reach real domains; dead-end mailbox, fake tld, absent root zone, register provider once, shared helper, no example.com, never resolves."
    use_when: "Entity carries an email field; realistic address must never reach a real domain; project-wide fake provider should be registered once and shared."
    avoid_when: "`faker.email()` relied on with real-looking domains; `@example.com` hardcoded for email-bearing entities."
