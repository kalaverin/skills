...
index:
  - anchor: parametrization-parametrize-basics
    ...
    problem: "Writing one test per case duplicates identical bodies and hides which cases share assertion; static finite vectors, known expected, case ids, per-case mark, no if elif, input in parameter."
    use_when: "Static finite cases have known expected results at import time, one body should cover them, and ids plus marks describe rows; static finite vectors, known expected, case ids, per case mark, no if elif, input in parameter."
    avoid_when: "Avoid embedding `if`/`elif` logic that reconstructs inputs from a case label — put the real input into the parameter or into a fixture."
    expected: "One parametrized body covers every declared case with stable ids and per-case marks, and inputs live in parameters or fixtures rather than in-branch reconstruction."
  - anchor: parametrization-cartesian-product
    ...
    problem: "Multi-dimensional contracts (access-control matrices, feature-flag combos) need every dimension combination covered without hand-writing each row; stacked decorators, independent dimensions, full product, access matrix, explicit rows invalid, combination coverage."
    use_when: "Contract dimensions multiply independently, hand writing each row misses combinations, and stacked decorators generate full product; stacked decorators, independent dimensions, full product, access matrix, explicit rows invalid, combination coverage, cartesian product."
    avoid_when: "Avoid stacking when most combinations are invalid — explicit rows keep failure messages closer to business intent."
    expected: "Every combination of the independent dimensions runs as its own case, while mostly-invalid spaces are expressed as explicit rows for clearer failures."
  - anchor: parametrization-parametrized-fixtures
    ...
    problem: "Cross-cutting variations (backends, formats, transports) declared per-test duplicate same matrix across suite; shared matrix, fixture params, chain dimensions, one-off local, cross-cutting, whole suite feed."
    use_when: "Cross cutting variation feeds many tests, per test declaration duplicates matrix, and fixture params share one declaration; shared matrix, fixture params, chain dimensions, one off local, cross cutting, whole suite feed, backend format transport."
    avoid_when: "Avoid creating a parametrized fixture for a variation only one test cares about — keep local knowledge local."
  ...
  - anchor: parametrization-indirect
    ...
    problem: "Complex dependencies (authenticated clients, databases, feature-flag configs) must construct per case, which test cannot do from raw scalar; route to fixture, request param, build complex, guard hasattr, scalar direct, per-case construct."
    use_when: "Complex dependency must construct per case, raw scalar cannot build it, and indirect route sends parameter into fixture; route to fixture, request param, build complex, guard hasattr, scalar direct, per case construct, indirect."
    avoid_when: "Avoid indirect parametrization for simple scalar values the test can use unchanged."
  ...
  - anchor: parametrization-lazy-fixtures
    ...
    problem: "Parameter sometimes needs to be fixture instance (admin versus guest user), which plain values cannot express; fixture as parameter, resolve at collection, switch fixtures, non-fixture ordinary, plugin required, initialized instance."
    use_when: "Parameterized value must be fixture instance, plain values cannot express initialized object, and plugin resolves fixture at collection or runtime; fixture as parameter, resolve at collection, switch fixtures, non fixture ordinary, plugin required, initialized instance."
    avoid_when: "Avoid `lazy_fixture` for values that are not fixtures — pass them as ordinary parameters; require the `pytest-lazy-fixtures` plugin to be installed."
  ...
  - anchor: parametrization-dynamic
    ...
    problem: "Some case lists are not known until collection time and cannot be written as static decorator rows; collection-time data, cli env file, runtime discovery, filter by fixture, static in decorator, dynamic case list."
    use_when: "Case list is unknown until collection time, static decorator rows cannot express it, and hook builds cases from CLI env files or discovery; collection time data, cli env file, runtime discovery, filter by fixture, static in decorator, dynamic case list."
    avoid_when: "Avoid it for static matrices expressible directly in decorators — explicit decorators are easier to trace and reuse."
  ...
  - anchor: parametrization-anti-patterns
    ...
    problem: "Branching on case labels inside test and duplicating tests per backend, role, or environment both hide variation and bloat suite; builder mapping, expected mapping, parametrized fixture, no if elif ladder, no copy-paste, case library."
    use_when: "Input reconstruction ladders and copy pasted tests hide variation, builder and expected mappings plus parametrized fixtures expose cases; builder mapping, expected mapping, parametrized fixture, no if elif ladder, no copy paste, case library."
    avoid_when: "Do not let `if`/`elif` ladders reconstruct inputs inside a parametrized test (the branch logic belongs in a fixture, factory, or the table), and do not copy-paste near-identical tests for each backend, role, or environment."
