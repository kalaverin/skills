...
| **rtk-protocol** | Token-optimized command execution via RTK (Rust Token Killer): always-prefix golden rule, full dedicated-filter command inventory pinned to the installed rtk version, and the precedence contract with shell-protocol and the MCP tool layer. Activates when the `rtk` binary is in PATH. |
