...
| **mandatory-tools** | Mandates modern CLI tooling for filesystem, search, and Python operations: `lsd`, `fd`, `rg`, `ruplacer`, `uv`, `ruff`. Replaces legacy `ls`, `find`, `grep`, `sed`, `pip`, `black`, `flake8`, etc. |
| **rtk-protocol** | Token-optimized command execution via RTK (Rust Token Killer): always-prefix golden rule, full dedicated-filter command inventory pinned to the installed rtk version, and the precedence contract with mandatory-tools and the MCP tool layer. Activates when the `rtk` binary is in PATH. |
| **patchloom-protocol** | Structured file editing via Patchloom (MCP-first, CLI fallback): parser-backed JSON/YAML/TOML and markdown-section ops, batch/atomic multi-op plans with rollback, apply-honesty traps, version pin. Activates when the `patchloom` binary is in PATH; MCP rules live only while the Patchloom MCP server is connected. |
