...
| **feature-archival** | Archives a completed feature's Serena-memory footprint: context-budgeted subagent extraction, compression into a flat `archive/<feature>/` record (`summary.md` + `decisions.md` + `future.md` + free tree), a reconciliation verification pass, and recoverable deletion of the originals (checkpoint-hash recovery). Explicit closing command only. |
