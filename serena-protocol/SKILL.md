...
**Bulk import of external documents (carve-out).** Raw external documents (standards, specs — bodies NOT authored by the agent) MAY be imported by the file path instead of `write_memory`, because piping a large raw body through the agent context is waste. The ONLY sanctioned pipeline: (1) download into a scratch path (e.g. `.tmp/`) via `curl`/`wget`/`fetch`; (2) `mv` into the target scope under `.serena/memories/`; (3) a subagent validates and attests the document (type, completeness, cleanliness) and drafts the frontmatter — on attestation failure, delete the moved file, report to the user, and STOP (if no subagent is available, the main agent attests per `subagents-protocol` §12); (4) stamp the header with `edit_memory` (needle `\A`, repl = frontmatter + synthetic `# <Title>` line) — the raw body below stays untouched; (5) verify the header via the canonical frontmatter extraction (`frontmatter-protocol` §6); (6) run the persistence command. `write_memory` remains mandatory for ALL agent-authored content; this carve-out never covers edits, appends, or agent-written text.
