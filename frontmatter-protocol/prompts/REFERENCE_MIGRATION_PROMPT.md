...
Copy this prompt, fill the `{{...}}` inputs, and give it to the main agent. The main agent orchestrates; subagents execute per-file work. The output is a target skill whose entire `references/` corpus conforms to the lazyload standard of `frontmatter-protocol` (`frontmatter-protocol/references/lazyload.md`).
...
- The standard: `frontmatter-protocol/references/lazyload.md` (mirror: `.kimi/mirror/frontmatter-protocol/references/lazyload.md`). Every rule about cards, anchors, and body skeleton lives there; the loader contract, the envelope, and the delimiter law live in `frontmatter-protocol/SKILL.md` (core §7, §3–§4).
...
- After every Serena memory mutation: verify and persist per `serena-protocol` `[ref: #serena-memory-mutation]`.
...
3. Verify the repo gate: a repo card MUST exist at `repos/{{SKILL}}/overview` in Serena memory. If missing — STOP and ask the user to create it via `repo-audit`; do not proceed.
...
1. **Paths, not contents**: the target file path (real path for editing) and the mirror paths to read: `.kimi/mirror/frontmatter-protocol/references/lazyload.md` (File Layout, Body Skeleton, Anchors sections) and one conformant example `.kimi/mirror/pytest-design/references/required/mocking.md`.
2. **The transformation contract**:
   ...
   - Under every routable section heading place the marker `[ref: #<file-prefix>-<section-slug>]` at column 0, on its own line, per markdown-protocol `marker_style` (default **tight**: directly under the heading, one blank line below). kebab-case ids; `<file-prefix>` = filename with underscores → hyphens (one consistent prefix per file); `<section-slug>` = 1–4 words.
...
Goal: every file gets a **planned** (draft) frontmatter per `frontmatter-protocol/references/lazyload.md`.

Per file, a `coder` subagent prompt MUST contain:

1. Mirror paths to the standard (Frontmatter Schema, Index Cards, Firm Style for `problem`, Dedup & Convergence sections in full) and to 1–2 conformant pytest-design examples.
2. The file path; instruction to read it fully and enumerate every `[ref]` anchor.
3. Draft requirements: apply the loaded standard sections literally — `subject` per **Frontmatter Schema** (essence + cloud covering EVERY section area), cards per **Index Cards** + **Firm Style for `problem`** + **Dedup & Convergence** (exactly six keys per card, one card per anchor minimum, convergence by anchor repetition with distinct criteria paths, cross-field dedup checked singular/plural both ways).
...
4. End-to-end loader rehearsal: run Command 1 (`rg -N -H '^subject:'`), shortlist 2–3 files against a realistic request, run Command 2, select cards, and extract one section with the bounded `awk` extractor from the core loader mechanics (`frontmatter-protocol/SKILL.md` §7) — prove the funnel works on this corpus.
5. Check `- [x] P4` per file, checkpoint, report.

## Phase 5 — Summary and skill wiring

1. Update `{{SKILL_PATH}}/SKILL.md`: replace any static routing tables with the two-command funnel and the semantic context-aware routing rules (core loader mechanics, `frontmatter-protocol/SKILL.md` §7); point to the standard; keep the skill's own mandatory rules.
