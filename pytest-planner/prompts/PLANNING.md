...
> **Mode note:** this prompt is the `project` plan mode (whole-entity coverage). For a diff-scoped coverage plan of the current branch against its base branch, use `prompts/FEATURE.md` via the Plan Mode Resolution (HARD STOP) fork in `SKILL.md`.
...
Immediately after the Boot Sequence, before the inventory (Phase A), ask the user the following and STOP until answered. Record each answer in the plan.
...
> **Layer:** unit only
> **Scope:** Full project coverage plan (not diff-based)
