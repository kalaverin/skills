...
| `archive/` | `archive/<feature>/` | Archived completed-feature records: flat feature-keyed trees (fixed core `summary.md` + `decisions.md` + `future.md`, additive free tree, `scraps.md`) carrying `archived_from_commit` for git-based recovery of the deleted originals. One feature key covers all repos the feature spanned. Owner: `feature-archival`. |
...
- `archive/<feature>/`: archived completed-feature records, written only by the on-demand `feature-archival` pipeline on the user's explicit closing command. Flat and feature-keyed (a feature spans repos) — no `<repo>` component, so the prerequisite gate is inapplicable here.
