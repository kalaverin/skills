...
Reference implementation of the Cross-Skill Reference File & Frontmatter
Standard (`bootstrap/references/REFERENCE_STANDARD.md`, §9 checklist). Copied
from the hardened api-design checker (2026-07-22, P4 hardening) and generalized:
...
- `libraries` entries are banned from `problem` clouds (§4.4 cloud discipline).

Deep dedup checks (§4.4/§4.5):
...
- Exemption (§4.5): n-grams containing a backticked identifier token or a
  `(<file> › <topic>)` cross-reference pointer are exempt.

Normalization: stopwords removed, `ies->y` and one trailing "s" stripped
(singular/plural equivalence per §4.5), prefix matching (>=4 chars) for the
paraphrase check.

Usage:
    uv run --no-project --with pyyaml python validate_reference_frontmatter.py \
        [--allow-extra KEY]... [--aips] <FILE.md>
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("file", type=Path, help="reference .md file to validate")
    parser.add_argument(
        "--allow-extra",
        action="append",
        default=[],
