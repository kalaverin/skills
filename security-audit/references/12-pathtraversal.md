...
index:
  - anchor: pathtraversal-detection
    ...
    use_when: "Path-traversal scan selected by the screener; `{{ REPORTS_ROOT }}/01_architecture.md` exists; full three-stage detection must run."
  ...
  - anchor: pathtraversal-execution
    what: "Domain execution parameters for the shared three-stage protocol: recon catalog of file-loading sinks with dynamic paths, per-candidate taint verify checklist, classification rubric, and the finding-field set with taint trace and dynamic tests."
    problem: "Traversal hunting without precise domain criteria lets recon miss dynamic-path sinks and verify apply generic checklists that overlook taint flows and mitigation subtleties; criteria ownership, domain parameters, search catalog, checklist precision, detection quality, class specifics."
    use_when: "Dispatching or executing any pipeline stage for this scan; reviewing whether recon and verify criteria cover current path-traversal vectors."
    avoid_when: "Stage mechanics — batching, gating, merging — belong to `execution-protocol.md`; conceptual traversal knowledge belongs to the definition and examples anchors."
    expected: "Stage subagents apply exact traversal criteria without inheriting generic templates."
...
You are performing a focused security assessment to find path traversal vulnerabilities in a codebase. This skill uses a three-stage pipeline with subagents: **recon** (find file-loading sinks with dynamic paths), **batched verify** (trace user input and check mitigations in parallel batches of 3), and **merge** (consolidate batch results into one report).
...
This scan runs via the shared three-stage pipeline in `references/execution-protocol.md` (recon+split → per-batch verify → merge, core-dispatched). The domain parameters below plug into its stage contracts. Final artifact: `{{ REPORTS_ROOT }}/12_pathtraversal.md`; classification family: standard (`[VULNERABLE]` / `[LIKELY VULNERABLE]`).

### Recon catalog

Find every location in the codebase where a file is opened, read, served, or extracted using a dynamically constructed path — meaning the path (or a component of it) is stored in a variable rather than being a fully hardcoded string. Flag any call to a file-reading/serving function where the path argument contains a variable (regardless of where the variable comes from). Do **not** trace user input at the recon stage — that is the verify stage's job. Just find all dynamic file access patterns.

Search for these file-loading sinks with dynamic path components:
...
**Recon exclusions** — do not report (no dynamic path component):
...
For each candidate sink, determine whether a user-supplied value reaches the dynamic path variable AND whether any mitigation prevents the path from escaping the intended base directory. Path traversal means user-supplied input incorporated into a filesystem path without constraining the resolved path to an intended base directory (including ZipSlip-style archive extraction). Do **not** flag SSRF, pure RCE/file-write classes, fully hardcoded paths, or safe `realpath`/`resolve` + base prefix checks as path traversal (see "What Path Traversal is NOT" and "Patterns That Prevent Path Traversal" above).
...
Every finding block carries: classification tag, file/lines, endpoint or function, issue, taint trace (step-by-step from entry point to the file operation; best-effort with the uncertain step identified for likely findings), missing mitigation (for likely findings the concern despite partial mitigation; for not-vulnerable the reason; for manual review the uncertainty plus a manual-trace suggestion), impact (e.g., read arbitrary files accessible to the process user — `/etc/passwd`, application config, source code, private keys), remediation, and a dynamic test — a curl command or payload from the Dynamic-Test Payloads section confirming the traversal, showing encoded variants as appropriate. Findings are grouped by classification in the final report: `[VULNERABLE]` first, then `[LIKELY VULNERABLE]`, then `[NEEDS MANUAL REVIEW]`, then `[NOT VULNERABLE]`.
...
- **The recon stage is purely structural**: flag any file-loading sink where the path has a dynamic component, regardless of origin. Do not attempt to trace user input in the recon stage — that is the verify stage's job.
- **The verify stage is taint analysis + mitigation review**: for each sink found in the recon stage, (a) trace the path variable back to its origin and (b) check whether an effective mitigation prevents escape from the intended directory.
...
- Intermediate-file lifecycle is owned by `execution-protocol.md`: the merge stage deletes `12_recon.md`, `12_batch_*.md`, and `12_verify_*.md`; only the final `{{ REPORTS_ROOT }}/12_pathtraversal.md` persists.
