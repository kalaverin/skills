...
- After `{{ REPORTS_ROOT }}/report.md` is written, run `just agent-memory-commit` from the project root so the audit artifacts are persisted with the rest of Serena memory.
