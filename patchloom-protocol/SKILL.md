...
- **Server registration note (host-side):** the MCP root is pinned explicitly with `--cwd <project>` in the host's server entry (Zed `context_servers`), NOT left to the spawn cwd: the local `subrunner` wrapper re-cds children into the child binary's directory, which would silently mis-root the server (#1832 host contract: the host pins `--cwd`, the model never supplies it). MCP enforces the server root for every call (`AllowIfContained`).
